...
from app.api.auth_handlers import get_current_user, create_access_token
...
# Статические роуты должны быть перед динамическими
...
async def get_last_invoice(
    ...
    try:
        if current_user.last_invoice_id:
            invoice = await crud.fetch_invoice(session, current_user.last_invoice_id, current_user)
            return invoice
        raise HTTPException(
...
# Динамические роуты идут после статических
